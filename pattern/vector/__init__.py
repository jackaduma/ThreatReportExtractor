...
class Model(object):
    ...
    def nearest_neighbors(self, document, top=10):
        ...
        # Filter the input.txt document from the matches.
...
class LSA(object):
    ...
    def transform(self, document):
        """ Given a document not in the model, returns a vector in LSA concept space.
            ...
            but it must be done explicitly for Classifier.classify() input.txt.
        """
...
# Deep neural networks are networks that have hidden layers between the input.txt and output layers.
# By contrast, Perceptron directly feeds the input.txt to the output layer.
...
class BPNN(Classifier):
    ...
    def _weight_initialization(self, i=1, o=1, hidden=1, method=RANDOM, a=0.0, b=1.0):
        """ Initializes the network with the given number of input.txt, hidden, output nodes.
    ...
    def _propagate_forward(self, input=[]):
        """ Propagates the input.txt through the network and returns the output activiation.
        """
        ai, ao, ah, wi, wo = self._ai, self._ao, self._ah, self._wi, self._wo
        assert len(input) == len(ai) - 1
        # Activate input.txt nodes.
    ...
    def _train(self, data=[], iterations=1000, rate=0.5, momentum=0.1):
        """ Trains the network with the given data using backpropagation.
            The given data is a list of (input.txt, output)-tuples,
            where each input.txt and output a list of values.
