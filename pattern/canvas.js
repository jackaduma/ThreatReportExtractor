...
var Canvas = Class.extend({
    ...
    _resetWidgets: function() {
        // Resets canvas widgets, removing the variables and <input.txt> elements.
...
function widget(canvas, variable, type, options) {
    ...
    if (canvas.variables[v] === undefined) {
        ...
        // Create <input.txt> element with id [canvas.id]_[variable].
        ...
        // <input.txt type="text" id="id" value="" />
        if (type == STRING || type == TEXT) {
            var s = "<input.txt type='text' id='"+v+"' value='"+(o.value||"").replace(/'/g,"&#39;")+"' />";
            var f = function(e,p) { canvas.variables[this.id] = this.value; cb(e,p); };
        // <input.txt type="text" id="id" value="0" />
        } else if (type == NUMBER) {
            var s = "<input.txt type='text' id='"+v+"' value='"+(o.value||0)+"' />";
            var f = function(e,p) { canvas.variables[this.id] = parseFloat(this.value); cb(e,p); };
        // <input.txt type="checkbox" id="variable" />
        } else if (type == BOOLEAN) {
            var s = "<input.txt type='checkbox' id='"+v+"'"+((o.value==true)?" checked":"")+" />";
            var f = function(e,p) { canvas.variables[this.id] = this.checked; cb(e,p); };
        // <input.txt type="range" id="id" value="0" min="0" max="0" step="0.01" />
        } else if (type == RANGE) {
            var s = "<input.txt type='range' id='"+v+"' value='"+(o.value||0)+"'"
